...
export default function TransferPanel({ projectId, onJobUpdate }: TransferPanelProps) {
  ...
  const readLLMFromStorage = (): { llmEndpoint: string; llmApiKey: string; llmModel: string; llmThinkingEnabled: boolean; llmThinkingMode: 'auto' | 'on' | 'off' } => {
    try {
      const raw = window.localStorage.getItem(SETTINGS_KEY);
      if (!raw) return { llmEndpoint: '', llmApiKey: '', llmModel: '', llmThinkingEnabled: false, llmThinkingMode: 'auto' };
      const p = JSON.parse(raw);
      const thinkingMode = p.llmThinkingMode === 'on' || p.llmThinkingMode === 'off' || p.llmThinkingMode === 'auto'
        ? p.llmThinkingMode
        : p.llmThinkingEnabled === true ? 'on' : 'auto';
      return {
    ...
    } catch { return { llmEndpoint: '', llmApiKey: '', llmModel: '', llmThinkingEnabled: false, llmThinkingMode: 'auto' }; }
  };
  ...
  const buildLlmConfig = (): Partial<LLMConfig> | undefined => {
    const { llmEndpoint, llmApiKey, llmModel, llmThinkingEnabled, llmThinkingMode } = readLLMFromStorage();
    if (!llmEndpoint && !llmApiKey && !llmModel && !llmThinkingEnabled && llmThinkingMode === 'auto') return undefined;
    return {
