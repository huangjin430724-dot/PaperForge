...
const COLLAB_SERVER_KEY = 'PaperForge-collab-server';
...
export function callLLM(payload: {
  ...
  return request<{ ok: boolean; content?: string; error?: string; usage?: Record<string, unknown> | null; thinking?: Record<string, unknown> | null }>('/api/llm', {
    method: 'POST',
    body: JSON.stringify(payload)
  });
}
