...
import { extractArxivId, fetchArxivEntry, buildArxivBibtex, searchArxiv } from './arxivService.js';
...
export async function runToolAgent({
  ...
  if (!resolved.apiKey) {
    return { ok: false, reply: 'PaperForge_LLM_API_KEY not set', patches: [] };
  }

  const createExecutor = async (thinkingEnabled) => {
  ...
  const system = [
    'You are a LaTeX paper assistant for PaperForge.',
  ...
  const thinkingDecision = resolveThinkingDecision({
    llmConfig: resolved,
    endpoint: resolved.endpoint,
    model: resolved.model,
  ...
  let executor = await createExecutor(thinkingDecision.enabled);
  let result;
  try {
    result = await executor.invoke({ input: userInput });
  } catch (err) {
    if (!thinkingDecision.enabled || !/reasoning_content|thinking mode/i.test(String(err))) {
      throw err;
    }
    executor = await createExecutor(false);
    result = await executor.invoke({ input: userInput });
  }
