...
export function resolveMineruConfig(mineruConfig) {
  const rawBase = (mineruConfig?.apiBase || process.env.PaperForge_MINERU_API_BASE || MINERU_API_BASE).trim();
  ...
  return {
    ...
    token: (mineruConfig?.token || process.env.PaperForge_MINERU_TOKEN || '').trim(),
...
export async function parsePdfWithMineru(pdfPath, mineruConfig, outputDir, onProgress) {
  ...
  if (!token) {
    throw new Error('MinerU token not configured. Set PaperForge_MINERU_TOKEN or provide in settings.');
  }
  if (callback && !seed) {
